...
<body>
            ...
            <form action="" id='form' method='post'>
                ...
                <div class="row mt-3">
                    ...
                    <div class="col-11 px-0">
                        <input type='submit' class="btn text-center btn-light d-flex m-auto   bg-info save" name="submit" value="Save">
    ...
    <script>
        ...
        //                 $('#addingCon input#' + data).val(ui.draggable.clone().text());
        //                 return; // Exit from the entire block of code
        //             } else if (data == "starttime-1") {
        //                 $('#addingCon input#' + data).val(ui.draggable.clone().text());
        //                 return;
        //             } else if (data.startsWith("endtime-")) {
        //                 // var number=data.charAt(str.length - 1);
        //                 var inputId = `starttime-${data.charAt(data.length - 1)}`;
        //                 let inputValue = $("#" + inputId).val();
        //                 if (ui.draggable.clone().text() >= inputValue) {
        //                     $('#addingCon input#' + data).val(ui.draggable.clone().text());
        //                 } else {
        //                     alert("end time should higher value than start tieme :)");
        //                     return;
        //                 }
        ...
        //                     $('#addingCon input#' + data).val(ui.draggable.clone().text());
        //                 } else {
        //                     alert("start  time should higher value than prev end tieme :)");
        //                     return;
        //                 }
        ...
        //                         $('#addingCon input#' + data).val(ui.draggable.clone().text());
        //                         return
        //                       }else if(data=='endminute-1' ){
        //                         alert($("#" + `startminute-1`).val() < ui.draggable.clone().text())
        //                         if($("#" + `startminute-1`).val() < ui.draggable.clone().text()) {
        //                         $('#addingCon input#' + data).val(ui.draggable.clone().text());
        //                         return;
        //                         }
        //                       }
        //                else if (($("#" + `starttime-${data.charAt(data.length - 1)}`).val() == $("#" + `endtime-${data.charAt(data.length - 1)}`).val()) ||
        //                     ($("#" + `starttime-${data.charAt(data.length - 1)}`).val() == $("#" + `endtime-${data.charAt(data.length - 1)-1}`).val())
        //                 ){
        //                     alert((($("#" + `starttime-${data.charAt(data.length - 1)}`).val() == $("#" + `endtime-${data.charAt(data.length - 1)}`).val()) ||
        //                     ($("#" + `starttime-${data.charAt(data.length - 1)}`).val() == $("#" + `endtime-${data.charAt(data.length - 1)-1}`).val())
        //                 ))
        //                       //////////////
        //                       if (data.startsWith("startminute-")){
        //                         if($("#" + `endminute-${data.charAt(data.length - 1)-1}`).val() < ui.draggable.clone().text()){
        //                             $('#addingCon input#' + data).val(ui.draggable.clone().text());
        //                             return;
        //                         }
        //                       }else if(data.startsWith("endminute-")){
        //                         if(($("#" + `startminute-${data.charAt(data.length - 1)}`).val() < ui.draggable.clone().text())){
        //                             $('#addingCon input#' + data).val(ui.draggable.clone().text());
        //                             return;
        //                         }
        //                       }
        ...
        //////////////////////////////////////
        function dragAndDrop($element) {
    $element.draggable({ helper: 'clone' }).droppable({
        accept: 'li',
        drop: function(event, ui) {
            var data = $(this).attr('id');
            ...
            if (data.startsWith("work-") || data == "starttime-1") {
                ...
                if(data=="startminute-1") {
                    $('#addingCon input#' + data).val(ui.draggable.clone().text());
                } else if(data=='endminute-1' && getValue(`startminute-1`) < ui.draggable.clone().text()) {
                    $('#addingCon input#' + data).val(ui.draggable.clone().text());
                } else if ((getValue(`starttime-${data.charAt(data.length - 1)}`) == getValue(`endtime-${data.charAt(data.length - 1)}`)) ||
                            (getValue(`starttime-${data.charAt(data.length - 1)}`) == getValue(`endtime-${data.charAt(data.length - 1)-1}`))) {
                    if ((data.startsWith("startminute-") && getValue(`endminute-${data.charAt(data.length - 1)-1}`) < ui.draggable.clone().text()) ||
                        (data.startsWith("endminute-") && getValue(`startminute-${data.charAt(data.length - 1)}`) < ui.draggable.clone().text())) {
                        $('#addingCon input#' + data).val(ui.draggable.clone().text());
                    }
                } else {
                    $('#addingCon input#' + data).val(ui.draggable.clone().text());
